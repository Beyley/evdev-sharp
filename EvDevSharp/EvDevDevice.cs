...
public sealed unsafe partial class EvDevDevice : IDisposable {
    ...
    private EvDevGuessedDeviceType GuessDeviceType() {
        if (this.Name != null) {
            ...
            if (n.Contains("touchscreen") && isAbsolutePointingDevice && this.Keys?.Contains(EvDevKeyCode.BtnTouch) == true)
                return EvDevGuessedDeviceType.TouchScreen;

            if (n.Contains("tablet") && isAbsolutePointingDevice && this.Keys?.Contains(EvDevKeyCode.BtnLeft) == true)
                return EvDevGuessedDeviceType.Tablet;

            if (n.Contains("touchpad") && isAbsolutePointingDevice && this.Keys?.Contains(EvDevKeyCode.BtnLeft) == true)
                return EvDevGuessedDeviceType.TouchPad;

            if (n.Contains("keyboard") && this.Keys != null)
                return EvDevGuessedDeviceType.Keyboard;

            if (n.Contains("gamepad") || n.Contains("joystick") && this.Keys != null)
                return EvDevGuessedDeviceType.GamePad;
        }

        if (this.Keys?.Contains(EvDevKeyCode.BtnTouch) == true && this.Properties.Contains(EvDevProperty.InputPropDirect))
            return EvDevGuessedDeviceType.TouchScreen;
    ...
    public static IEnumerable<EvDevDevice> GetDevices() {
        ...
        return Directory.GetFiles(INPUT_PATH, INPUT_PATH_SEARCH_PATTERN).AsParallel().Select(path => new EvDevDevice(path));
    }
}
