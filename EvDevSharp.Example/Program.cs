...
devices[key].OnKeyEvent += (s, e) => { Console.WriteLine($"Key: {e.Key}\tValue: {e.Value}"); };

devices[key].OnRelativeEvent += (s, e) => { Console.WriteLine($"Rel Axis: {e.Axis}\tValue: {e.Value}"); };

devices[key].OnAbsoluteEvent += (s, e) => { Console.WriteLine($"Abs Axis: {e.Axis}\tValue: {e.Value}"); };
